...
**❌ DAI ≥ 2 is worse than random — *as a standalone gate*.** 0.965×. We shipped it. The data says it was a coin flip with extra steps.

That qualifier is load-bearing, and it is the half of our own result that is inconvenient for us. Run the same numbers *conditional on presentation* — i.e. inside the 1,370 candidates our real rule already keeps — and the differential stops being useless:

| Rule, applied **within** the 1,370 presented | Kept | Responders | Precision |
|---|---:|---:|---:|
| *(baseline — all presented)* | 1,370 | 49 | **3.58%** |
| DAI ≥ 10 | 204 | 10 | **4.90%** ✅ |
| DAI ≥ 2 | 446 | 13 | 2.91% ❌ |
| anchor mutation | 331 | 14 | **4.23%** |
| TCR-facing mutation | 1,039 | 35 | 3.37% |

Two honest readings of this, and we cannot yet distinguish them:

1. **DAI is a weak *re-ranker*, not a gate.** A hard threshold on the differential throws away responders faster than it throws away non-responders, so it loses as a filter. But at a *strict* cut (≥ 10) on an already-presented set it lifts precision 3.58% → 4.90%. Our own error was picking both the wrong threshold (2) and the wrong role (gate).

2. **204 candidates and 10 responders is a small enough cell that this is noise.** 10 vs the ~7.3 responders you would expect by chance is not significant at any threshold we would be willing to defend. We are not claiming a win here.

**Note also that anchor mutations out-precision TCR-facing ones** (4.23% vs 3.37%), which is *backwards* from the mechanistic story we tell in §1.1 and in `docs/SCIENCE.md` — the story that anchor mutations point into the groove where no T-cell receptor can reach them. Either the mechanism is weaker than the textbook version, or presentation scoring is already absorbing the anchor signal, or this is the same small-*n* noise. **We do not know, and we would rather print the contradiction than the tidy version.** This block is generated by `scripts/validate_screen.py` into `benchmarks/screen_validation.json` under `conditional_on_presentation`, so it cannot quietly drift away from the claim above.
